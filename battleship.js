var model = {
	...
	generateShip: function() {
		...
		if (direction === 1) {
			...
			col = Math.floor(Math.random() * (this.gridSize - this.shipLength + 1));
		} else {
			col = Math.floor(Math.random() * this.gridSize);
			row = Math.floor(Math.random() * (this.gridSize - this.shipLength + 1));
		}
	...
	processGuess: function(guess) {
		...
		if (location) {
			...
			if (hit && model.shipSunk === model.numShips) {
				view.displayMessage("You sank all the Destroyers in " + this.guesses + " guesses.");
			}
		} 
	}
};
...
function init() {
	...
	var guessInput = document.getElemntById("guessInput");
